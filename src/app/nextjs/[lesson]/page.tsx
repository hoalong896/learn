...
export async function generateStaticParams() {
  return nextjsLessons.map(l => ({ lesson: l.id }));
}

export default async function NextjsLessonPage({ params }: Props) {
  const { lesson: lessonId } = await params;
  const lessonIndex = nextjsLessons.findIndex(l => l.id === lessonId);
  if (lessonIndex === -1) notFound();

  const lesson = nextjsLessons[lessonIndex];
  const prev = nextjsLessons[lessonIndex - 1];
  const next = nextjsLessons[lessonIndex + 1];
  ...
  return (
    <div className="flex min-h-screen bg-gray-950">
        ...
        <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-gray-500 text-sm">
                Bài {lessonIndex + 1} / {nextjsLessons.length}
              </span>
            </div>
          ...
          <div className="mb-6">
            <h2 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
              <span className="text-purple-400">⟨/⟩</span> Ví dụ minh họa — chạy thử ngay!
            </h2>
          ...
          <div className="flex justify-end mt-8">
            <CompleteButton course="nextjs" lessonId={lesson.id} courseColor="purple" />
          </div>

          {/* Navigation */}
          <div className="flex justify-between mt-6 pt-6 border-t border-gray-800">
            {prev ? (
              <Link href={`/nextjs/${prev.id}`}
            ...
            {next ? (
              ...
              <Link href="/nextjs" className="text-sm text-purple-400 hover:text-purple-300">
                Hoàn thành khóa học 🎉
              </Link>
            )}
          </div>
      ...
      <ChatWidget
        context={{ lessonTitle: lesson.title, courseTitle: "Next.js", lessonId: lesson.id }}
        courseColor="purple"
      />
    </div>
  );
