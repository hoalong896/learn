...
export async function generateStaticParams() {
  return kidsLessons.map(l => ({ lesson: l.id }));
}

export default async function KidsLessonPage({ params }: Props) {
  const { lesson: lessonId } = await params;
  const lessonIndex = kidsLessons.findIndex(l => l.id === lessonId);
  if (lessonIndex === -1) notFound();

  const lesson = kidsLessons[lessonIndex];
  const prev = kidsLessons[lessonIndex - 1];
  const next = kidsLessons[lessonIndex + 1];
  ...
  return (
    <div className="flex min-h-screen bg-gray-950">
      <LessonSidebar
        lessons={kidsLessons}
        coursePath="kids"
        courseTitle="Lập trình cho trẻ em"
        courseColor="yellow"
      />
        ...
        <div className="max-w-4xl mx-auto px-6 py-8">
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-gray-500 text-sm">
                Bài {lessonIndex + 1} / {kidsLessons.length}
              </span>
            </div>
          ...
          <div className="mb-6">
            <h2 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
              <span className="text-yellow-400">⟨/⟩</span> Ví dụ minh họa — chạy thử ngay!
            </h2>
          ...
          <div className="flex justify-end mt-8">
            <CompleteButton course="kids" lessonId={lesson.id} courseColor="yellow" />
          </div>

          <div className="flex justify-between mt-6 pt-6 border-t border-gray-800">
            {prev ? (
              <Link href={`/kids/${prev.id}`}
            ...
            {next ? (
              ...
              <Link href="/kids" className="text-sm text-yellow-400 hover:text-yellow-300">
                Hoàn thành khóa học 🎉
              </Link>
            )}
          </div>
      ...
      <ChatWidget
        context={{ lessonTitle: lesson.title, courseTitle: "Lập trình cho trẻ em", lessonId: lesson.id }}
        courseColor="yellow"
      />
    </div>
  );
