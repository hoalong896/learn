...
export async function generateStaticParams() {
  return javaLessons.map(l => ({ lesson: l.id }));
}

export default async function JavaLessonPage({ params }: Props) {
  const { lesson: lessonId } = await params;
  const lessonIndex = javaLessons.findIndex(l => l.id === lessonId);
  if (lessonIndex === -1) notFound();

  const lesson = javaLessons[lessonIndex];
  const prev = javaLessons[lessonIndex - 1];
  const next = javaLessons[lessonIndex + 1];
  ...
  return (
    <div className="flex min-h-screen bg-gray-950">
        ...
        <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-gray-500 text-sm">
                Bài {lessonIndex + 1} / {javaLessons.length}
              </span>
            </div>
          ...
          <div className="mb-6">
            <h2 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
              <span className="text-orange-400">⟨/⟩</span> Ví dụ minh họa — chạy thử ngay!
            </h2>
            <CodeEditor initialCode={lesson.codeExample} language="java" height="320px" />
          </div>

          {/* Exercises */}
          {lesson.exercises.map((ex, i) => (
            <ExercisePanel key={i} exercise={ex} exerciseNumber={i + 1} language="java" />
          ))}
          ...
          {lesson.playgrounds && lesson.playgrounds.length > 0 && (
            <div className="mt-10">
              <h2 className="text-lg font-bold text-white mb-1 flex items-center gap-2">
                <span className="text-orange-400">🎮</span> Sandbox — Thử nghiệm tự do
              </h2>
              <p className="text-sm text-gray-400 mb-4">Không có lời giải mẫu — hãy thử nghiệm và khám phá!</p>
              {lesson.playgrounds.map((pg, i) => (
                <PlaygroundPanel key={i} playground={pg} index={i} language="java" />
              ))}
            </div>
          )}
          ...
          <div className="flex justify-end mt-8">
            <CompleteButton course="java" lessonId={lesson.id} courseColor="orange" />
          </div>

          {/* Navigation */}
          <div className="flex justify-between mt-6 pt-6 border-t border-gray-800">
            {prev ? (
              <Link href={`/java/${prev.id}`}
            ...
            {next ? (
              ...
              <Link href="/java" className="text-sm text-orange-400 hover:text-orange-300">
                Hoàn thành khóa học 🎉
              </Link>
            )}
          </div>
      ...
      <ChatWidget
        context={{ lessonTitle: lesson.title, courseTitle: "Java", lessonId: lesson.id }}
        courseColor="orange"
      />
    </div>
  );
